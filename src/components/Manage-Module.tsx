      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-3 sm:gap-4">
          <div className="min-w-0 flex-1">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">Manage Modules</h1>
            <p className="text-xs sm:text-sm text-gray-500 mt-1">Modules belong to tracks and group related units</p>
          </div>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full sm:w-auto shrink-0">
            ...
            <span className="text-xs sm:text-sm text-gray-400 whitespace-nowrap bg-white border border-gray-200 px-3 py-2 rounded-xl text-center">
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 p-3 sm:p-4 lg:hidden">
                {filteredModules.map((mod) => {
                  ...
                  return (
                    <div key={mod.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex gap-3">
                        <div className="w-12 h-12 rounded-xl border border-gray-100 bg-gray-50 overflow-hidden flex items-center justify-center shrink-0">
                          {thumb ? <img src={thumb} alt="" className="w-full h-full object-cover" /> : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="font-semibold text-gray-900 text-sm truncate">{mod.title}</div>
                          {mod.shortDescription && <div className="text-xs text-gray-400 mt-0.5 line-clamp-2">{mod.shortDescription}</div>}
                          <div className="flex flex-wrap items-center gap-1.5 mt-2">
                            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium bg-blue-50 text-blue-700 border border-blue-100 truncate max-w-[140px]">{mod.track?.title ?? `Track #${mod.trackId}`}</span>
                            <Badge status={mod.status} />
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 mt-4">
                        <button onClick={() => setModal({ type: "addUnit", module: mod })} className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-[#004900] text-white hover:bg-[#003700] transition-colors">+ Unit</button>
                        <button onClick={() => setModal({ type: "addAssessment", module: mod })} className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-purple-600 text-white hover:bg-purple-700 transition-colors">+ Assessment</button>
                        <ActionsMenu items={[{ label: "Add Reflection", onClick: () => setModal({ type: "addReflection", module: mod }) },{ label: "Edit Reflection", onClick: () => setModal({ type: "editReflection", module: mod }) },{ label: "View Reflection", onClick: () => setModal({ type: "viewReflection", module: mod }) },{ label: "Edit Module", onClick: () => setModal({ type: "edit", module: mod }) },{ label: "Delete Module", onClick: () => setModal({ type: "delete", module: mod }), danger: true }]} />
                      </div>
                    </div>
                  );
                })}
